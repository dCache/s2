...
echo "<TR><TH BGCOLOR=OLIVE COLSPAN=30 HEIGHT=40>" >> ${Index_File}
...
do
   ...
   if test -z "$s2" ; then
     ...
     if test $s2_num -ge 70 -a $delim -eq 2; then
       echo "<TH BGCOLOR=OLIVE COLSPAN=30 HEIGHT=40>" >> ${Index_File} 
       echo "WLCG non MoU SRM v2.2 methods</TH></TR><TR>" >> ${Index_File}
       delim=0
     fi
     if test $s2_num -ge 62 -a $delim -eq 1; then
       echo "<TH BGCOLOR=ORANGE COLSPAN=30 HEIGHT=40>" >> ${Index_File}
