...
srmPutDone::init()
{
  ...
  /* response (API) */
  resp = new srm__srmPutDoneResponse_();
  if(resp == NULL) {
    DM_ERR(ERR_SYSTEM, "new failed\n");
  } else {
    memset(resp, 0, sizeof(srm__srmPutDoneResponse_));
  }
}
...
srmPutDone::exec(Process *proc)
{
  ...
  match = proc->e_match(fileStatuses, arrayOfPutDoneResponseToString(FALSE, FALSE).c_str());
  if(!match) {
...
srmPutDone::toString(Process *proc)
{
  ...
  std::vector <std::string *> surlArray =
    proc? proc->eval_vec_str(srmPutDone::surlArray):
          srmPutDone::surlArray;
  ...
  ss << arrayOfPutDoneResponseToString(TRUE, quote);
...
std::string
srmPutDone::arrayOfPutDoneResponseToString(BOOL space, BOOL quote) const
{
  DM_DBG_I;
  std::stringstream ss;
