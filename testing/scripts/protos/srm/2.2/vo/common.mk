...
echo -n "  <TD VALIGN=top ALIGN=center>" >> $1; if test -s "$(S2_LOGS_DIR)/$$s2_bare-$(VO)-$(VOTOKENTAG)-$(VOPATHTAG).e1" ; then echo -en "<A HREF=$$s2_bare-$(VO)-$(VOTOKENTAG)-$(VOPATHTAG).e1>after exec</A>" >> $1; else echo -en "n/a" >> $1; fi; echo "</TD>" >> $1;\
echo -n "  <TD VALIGN=top ALIGN=center>" >> $1; if test -s "$(S2_LOGS_DIR)/$$s2_bare-$(VO)-$(VOTOKENTAG)-$(VOPATHTAG).out" ; then echo -en "<A HREF=$$s2_bare-$(VO)-$(VOTOKENTAG)-$(VOPATHTAG).out>stdout<A>" >> $1; else echo -en "n/a" >> $1; fi; echo "</TD>" >> $1;\
echo -n "  <TD VALIGN=top ALIGN=center>" >> $1; if test -s "$(S2_LOGS_DIR)/$$s2_bare-$(VO)-$(VOTOKENTAG)-$(VOPATHTAG).log" ; then echo -en "<A HREF=$$s2_bare-$(VO)-$(VOTOKENTAG)-$(VOPATHTAG).log>custom log<A>" >> $1; else echo -en "n/a" >> $1; fi; echo "</TD>" >> $1;\
echo -e "</TR>" >> $1
